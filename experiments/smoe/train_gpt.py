...
class SoftMoEMLP(nn.Module):
    """Soft MoE: all experts process all tokens, weighted by soft routing.
    No sparse routing → compile-friendly. N experts each with mlp_mult expansion."""
    def __init__(self, dim: int, mlp_mult: int, num_experts: int = 4):
        super().__init__()
        hidden = mlp_mult * dim
        self.num_experts = num_experts
        # Each expert is a separate MLP
        self.experts = nn.ModuleList([MLP(dim, mlp_mult) for _ in range(num_experts)])
        # Soft router: per-token weights over experts
        self.router = CastedLinear(dim, num_experts, bias=False)
        nn.init.normal_(self.router.weight, std=0.01)

    def forward(self, x: Tensor) -> Tensor:
        # Compute soft routing weights
        weights = torch.softmax(self.router(x), dim=-1)  # [B, T, E]
        # Run all experts and combine (compile-friendly: fixed number of experts)
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        _num_experts = int(os.environ.get("NUM_EXPERTS", 0))
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init)
        if _num_experts > 0:
            self.mlp = SoftMoEMLP(dim, mlp_mult, num_experts=_num_experts)
        else:
