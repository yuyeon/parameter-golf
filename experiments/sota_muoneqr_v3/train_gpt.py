    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for i, m in enumerate(self._bank_meta):
                ...
                # MuonEq-R: row-normalize before Newton-Schulz (equivariance fix)
                row_norms = update.flatten(0, -2).norm(dim=-1, keepdim=True).clamp(min=1e-8)
                update = update.flatten(0, -2) / row_norms
