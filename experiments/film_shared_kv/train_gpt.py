...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        shared_kv: bool = False,  # if True, K/V come from external shared layers
    ):
        super().__init__()
        if dim % num_heads != 0:
    ...
    def forward(self, x: Tensor, shared_k_proj: nn.Module | None = None, shared_v_proj: nn.Module | None = None) -> Tensor:
        bsz, seqlen, dim = x.shape
        q = self.c_q(x).reshape(bsz, seqlen, self.num_heads, self.head_dim)
        k_proj = shared_k_proj if self.shared_kv else self.c_k
        v_proj = shared_v_proj if self.shared_kv else self.c_v
        k = k_proj(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
        v = v_proj(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init,
                                        rope_dims=rope_dims, train_seq_len=train_seq_len, shared_kv=shared_kv)
        self.mlp = MLP(dim, mlp_mult)

    def forward(self, x: Tensor, x0: Tensor, attn_scale: Tensor, mlp_scale: Tensor, resid_mix: Tensor,
                shared_k_proj: nn.Module | None = None, shared_v_proj: nn.Module | None = None) -> Tensor:
        mix = resid_mix.to(dtype=x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = self.attn(self.attn_norm(x), shared_k_proj=shared_k_proj, shared_v_proj=shared_v_proj)
...
class GPT(nn.Module):
    def __init__(
        ...
        share_kv = bool(int(os.environ.get("SHARE_KV", "0")))  # share K/V across blocks
        ...
        # Shared K/V projections: one K and one V used by all blocks
        self.share_kv = share_kv
        if share_kv:
            kv_dim = num_kv_heads * (model_dim // num_heads)
            self.shared_k = CastedLinear(model_dim, kv_dim, bias=False)
            self.shared_v = CastedLinear(model_dim, kv_dim, bias=False)
        ...
        if num_shared > 0:
            ...
            self.blocks = nn.ModuleList([
                Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                      rope_dims=rope_dims, train_seq_len=train_seq_len, shared_kv=share_kv)
    ...
    def _encode(self, input_ids: Tensor) -> Tensor:
        ...
        x0 = x
        skips: list[Tensor] = []
        sk = self.shared_k if self.share_kv else None
        sv = self.shared_v if self.share_kv else None
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[bi](x, x0, self.film.attn_scales[i], self.film.mlp_scales[i], self.film.resid_mixes[i],
                                shared_k_proj=sk, shared_v_proj=sv)
            skips.append(x)
