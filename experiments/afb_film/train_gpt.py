...
class Block(nn.Module):
    ...
    When attn_free=True, skips attention (MLP-only block for speed)."""
    def __init__(
        self,
        dim: int,
        num_heads: int,
...
class GPT(nn.Module):
    def __init__(
        ...
        # ATTN_FREE_BLOCKS: comma-separated indices of shared blocks that skip attention (MLP-only)
        afb_str = os.environ.get("ATTN_FREE_BLOCKS", "")
        attn_free_set = set(int(x) for x in afb_str.split(",") if x.strip()) if afb_str.strip() else set()
        # BLOCK_SCHEDULE: optional custom schedule (comma-separated block indices per virtual layer)
        sched_str = os.environ.get("BLOCK_SCHEDULE", "")
        ...
        if num_shared > 0:
            ...
            self.blocks = nn.ModuleList([
                Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                      attn_free=(i in attn_free_set))
                for i in range(num_shared)
            ])
            self.film = FiLMDepthParams(model_dim, effective_depth)
            if sched_str.strip():
                self.block_schedule = [int(x) for x in sched_str.split(",")]
                assert len(self.block_schedule) == effective_depth, \
                    f"BLOCK_SCHEDULE length {len(self.block_schedule)} != NUM_LAYERS {effective_depth}"
            else:
