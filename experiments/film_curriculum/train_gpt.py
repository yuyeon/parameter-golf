...
class Hyperparameters:
    ...
    curriculum_start_seq = int(os.environ.get("CURRICULUM_START_SEQ", 256))  # start with short seqs
    curriculum_switch_frac = float(os.environ.get("CURRICULUM_SWITCH_FRAC", 0.4))  # switch to full at 40% time
...
def main() -> None:
    ...
    while True:
        ...
        # Curriculum: use short sequences early, full length later
        if max_wallclock_ms is not None and args.curriculum_start_seq < args.train_seq_len:
            time_frac = elapsed_ms / max_wallclock_ms
            if time_frac < args.curriculum_switch_frac:
                curr_seq_len = args.curriculum_start_seq
            else:
                curr_seq_len = args.train_seq_len
                if step > 0 and not hasattr(main, '_curriculum_switched'):
                    main._curriculum_switched = True
                    log0(f"curriculum:switched to seq_len={curr_seq_len} at step:{step} time_frac:{time_frac:.3f}")
        else:
            curr_seq_len = args.train_seq_len
