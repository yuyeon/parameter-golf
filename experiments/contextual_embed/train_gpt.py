...
class GPT(nn.Module):
    def __init__(
        ...
        self.ctx_conv_k = int(os.environ.get("CTX_CONV_K", 0))  # causal conv kernel size (0=disabled)
        self.aux_loss_weight = float(os.environ.get("AUX_LOSS_WEIGHT", 0.0))  # auxiliary mid-layer prediction

        self.tok_emb = nn.Embedding(vocab_size, model_dim)
        if self.ctx_conv_k > 0:
            # Causal depthwise conv for contextual embeddings
            self.ctx_conv = nn.Conv1d(model_dim, model_dim, self.ctx_conv_k,
                                       padding=0, groups=model_dim, bias=False)
            nn.init.normal_(self.ctx_conv.weight, std=0.02)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.ctx_conv_k > 0:
            # Causal depthwise conv: left-pad then convolve (no future leakage)
            xt = x.transpose(1, 2)  # [B, D, T]
            xt = F.pad(xt, (self.ctx_conv_k - 1, 0))  # causal left-pad
            ctx = self.ctx_conv(xt)  # [B, D, T]
            x = x + ctx.transpose(1, 2)
        ...
        # Auxiliary mid-layer prediction at the bottleneck
        aux_loss = torch.tensor(0.0, device=x.device)
        if self.aux_loss_weight > 0:
            mid_x = self.final_norm(x).reshape(-1, x.size(-1))
            targets_flat = target_ids.reshape(-1)
            if self.tie_embeddings:
                mid_logits = F.linear(mid_x, self.tok_emb.weight)
            else:
                mid_logits = self.lm_head(mid_x) if self.lm_head is not None else mid_x
            mid_logits = self.logit_softcap * torch.tanh(mid_logits / self.logit_softcap)
            aux_loss = F.cross_entropy(mid_logits.float(), targets_flat, reduction="mean")
