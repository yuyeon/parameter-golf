...
class Hyperparameters:
    ...
    xsa_last_n = int(os.environ.get("XSA_LAST_N", 4))  # apply XSA to last N layers
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.use_xsa:
            y_bthd = y.transpose(1, 2)  # [B, T, H, D]
            v_bthd = v.transpose(1, 2)  # [B, T, Hkv, D]
...
class GPT(nn.Module):
    def __init__(
        ...
        xsa_n = int(os.environ.get("XSA_LAST_N", 4))
        for i in range(max(0, num_layers - xsa_n), num_layers):
    ...
    def _init_weights(self) -> None:
        ...
        for module in self.modules():
            ...
            elif isinstance(module, (nn.Linear, CastedLinear)) and not getattr(module, "_zero_init", False):
                if module.weight.ndim == 2:
                    nn.init.orthogonal_(module.weight)

    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        x = self.tok_emb(input_ids)
        # BigramHash: XOR-hash consecutive token pairs
        t = input_ids.to(torch.int32)
        mod = self.bigram_buckets_val - 1
        bigram_ids = torch.empty_like(t)
        bigram_ids[:, 0] = mod
        bigram_ids[:, 1:] = (36313 * t[:, 1:] ^ 27191 * t[:, :-1]) % mod
        x = x + self.bigram_proj(self.bigram_emb(bigram_ids.long()))
