        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(params):
                if i % world_size == rank and p.grad is not None:
                    ...
                    # Configurable optimizer variant
                    ...
                    elif optim_mode == "spectral":
                        # Spectral Muon: normalize by spectral norm (power iteration)
                        row_norms = g.norm(dim=-1, keepdim=True).clamp(min=1e-8)
                        g = g / row_norms
                        # 2-step power iteration for top singular value
                        g_bf = g.bfloat16()
                        v = torch.randn(g_bf.size(-1), 1, device=g_bf.device, dtype=g_bf.dtype)
                        for _ in range(2):
                            u = g_bf @ v
                            u = u / (u.norm() + 1e-8)
                            v = g_bf.T @ u
                            v = v / (v.norm() + 1e-8)
                        sigma_max = (u.T @ g_bf @ v).item()
                        g = g / max(abs(sigma_max), 1e-8)
                    elif optim_mode == "soft":
                        # Muon-Soft: fewer NS5 steps (partial orthogonalization)
                        row_norms = g.norm(dim=-1, keepdim=True).clamp(min=1e-8)
                        g = g / row_norms
                        g = zeropower_via_newtonschulz5(g, steps=2)  # 2 instead of 5
                    elif optim_mode == "coleq":
                        # MuonEq-C: column-normalize instead of row-normalize
                        col_norms = g.norm(dim=-2, keepdim=True).clamp(min=1e-8)
                        g = g / col_norms
                        g = zeropower_via_newtonschulz5(g, steps=backend_steps)
                    elif optim_mode == "bieq":
                        # MuonEq-RC: row-normalize then column-normalize (Sinkhorn-like)
                        row_norms = g.norm(dim=-1, keepdim=True).clamp(min=1e-8)
                        g = g / row_norms
                        col_norms = g.norm(dim=-2, keepdim=True).clamp(min=1e-8)
                        g = g / col_norms
