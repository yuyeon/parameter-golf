        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(params):
                if i % world_size == rank and p.grad is not None:
                    ...
                    optim_mode = os.environ.get("OPTIM_MODE", "muoneqr")
                    if optim_mode == "muoneqr":
                        # Standard MuonEq-R: row-normalize + NS5
                        row_norms = g.norm(dim=-1, keepdim=True).clamp(min=1e-8)
                        g = g / row_norms
                        g = zeropower_via_newtonschulz5(g, steps=backend_steps)
                    elif optim_mode == "lowrank":
                        # Rank-k power iteration: top-k singular directions only
                        # Filters gradient noise, keeps signal. MUCH cheaper than NS5.
                        k = int(os.environ.get("RANK_K", "32"))
                        row_norms = g.norm(dim=-1, keepdim=True).clamp(min=1e-8)
                        g_n = g / row_norms
                        g_f = g_n.float()
                        m_dim, n_dim = g_f.shape
                        # Power iteration for top-k right singular vectors
                        V = torch.randn(n_dim, k, device=g_f.device, dtype=g_f.dtype)
                        for _ in range(3):  # 3 power iterations
                            U = g_f @ V  # m×k
                            U = U / (U.norm(dim=0, keepdim=True) + 1e-8)
                            V = g_f.T @ U  # n×k
                            V = V / (V.norm(dim=0, keepdim=True) + 1e-8)
                        # Rank-k update: U @ V^T (partial isometry)
                        g = (U @ V.T).to(g.dtype)
                    elif optim_mode == "spectral_momentum":
                        # Spectral momentum: maintain running subspace via EMA of G^TG
                        row_norms = g.norm(dim=-1, keepdim=True).clamp(min=1e-8)
                        g_n = (g / row_norms).float()
                        # EMA of gradient covariance G^TG
                        key = id(p)
                        if "cov_ema" not in state:
                            state["cov_ema"] = (g_n.T @ g_n) / g_n.shape[0]
                        else:
                            beta_cov = 0.9
                            state["cov_ema"] = beta_cov * state["cov_ema"] + (1 - beta_cov) * (g_n.T @ g_n) / g_n.shape[0]
                        # Whiten gradient using running covariance: G @ C^{-1/2}
                        # C^{-1/2} via NS5 on C (n×n, much cheaper than m×n)
                        C = state["cov_ema"]
                        C_isqrt = zeropower_via_newtonschulz5(C.bfloat16(), steps=5)
                        g = (g_n @ C_isqrt.float()).to(g.dtype)
                        # Note: G @ C^{-1/2} ≈ UV^T if C = VΣ²V^T → this IS Muon
                        # BUT the running average C_ema ≠ single-step G^TG, so it denoises!
                    elif optim_mode == "nsg":
                        # Null-Space Gradient: remove gradient component in W's column space
                        # For tall matrices (m>n), this biases toward learning NEW output directions
                        # ΔW = (I - WW†)G = G - W(W^TW)^{-1}W^TG
                        w = p.data.float()
                        m_dim, n_dim = w.shape
                        alpha_nsg = float(os.environ.get("NSG_ALPHA", "0.5"))
                        if m_dim > n_dim:
                            # Compute W^T G (n×n for square G columns, but G is m×n so W^T G is n×n)
                            WtG = w.T @ g.float()  # n×n
                            WtW = w.T @ w  # n×n
                            # Solve (W^TW + εI)x = W^TG via Cholesky
                            eps_reg = 1e-4 * torch.diag(WtW).mean()
                            WtW.diagonal().add_(eps_reg)
                            L = torch.linalg.cholesky(WtW)
                            x = torch.cholesky_solve(WtG, L)  # n×n
                            g_colspace = w @ x  # m×n: gradient component in W's column space
                            g = g.float() - alpha_nsg * g_colspace.to(g.dtype)
                        # Then apply standard MuonEq-R + NS5
                        row_norms = g.norm(dim=-1, keepdim=True).clamp(min=1e-8)
                        g = g / row_norms
                        g = zeropower_via_newtonschulz5(g, steps=backend_steps)
                    elif optim_mode == "wip":
                        # Weight-Inverse Preconditioned: amplify gradient in under-represented
                        # directions of W. G' = G @ (W^TW + εI)^{-1} before NS5
                        w = p.data.float()
                        WtW = w.T @ w  # n×n
                        eps_reg = 1e-4 * torch.diag(WtW).mean()
                        WtW.diagonal().add_(eps_reg)
                        L = torch.linalg.cholesky(WtW)
                        # G @ (W^TW + εI)^{-1}: solve X(W^TW+εI) = G → X = G @ inv
                        g_f = g.float()
                        g_precond = torch.cholesky_solve(g_f.T, L).T  # (m,n)
                        # Blend with original gradient to avoid instability
                        beta_wip = float(os.environ.get("WIP_BETA", "0.3"))
                        g = ((1 - beta_wip) * g.float() + beta_wip * g_precond).to(g.dtype)
                        # Then apply standard MuonEq-R + NS5
                        row_norms = g.norm(dim=-1, keepdim=True).clamp(min=1e-8)
                        g = g / row_norms
                        g = zeropower_via_newtonschulz5(g, steps=backend_steps)
                    elif optim_mode == "procrustes":
                        # Procrustes: multiplicative update via rotation
                        # Find rotation R that best incorporates gradient into current weights
                        # R = polar_factor(W - lr*G) @ W†, then update W ← R @ W
                        # Simplified: compute polar factor of (W - G) and use it
                        w = p.data.float()
                        target = w - g.float()  # where we want to go
                        # Polar factor of target: U_t V_t^T
                        g = zeropower_via_newtonschulz5(target.to(g.dtype), steps=backend_steps)
                        # The update is: target_ortho - W_ortho (change in orthogonal structure)
                        w_ortho = zeropower_via_newtonschulz5(w.to(g.dtype), steps=backend_steps)
                        g = g - w_ortho.to(g.dtype)
                    else:
