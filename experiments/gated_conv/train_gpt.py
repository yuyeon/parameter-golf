...
class GatedConvMixer(nn.Module):
    """Gated causal convolution + cumulative mean for sequence mixing.
    Replaces attention: O(k*n*d) local conv + O(n*d) global mean vs O(n²*d) attention."""
    def __init__(self, dim: int, kernel_size: int = 15):
        super().__init__()
        self.kernel_size = kernel_size
        # Depthwise causal conv for local patterns
        self.conv = nn.Conv1d(dim, dim, kernel_size, padding=0, groups=dim, bias=False)
        nn.init.normal_(self.conv.weight, std=0.02)
        # Gate: controls how much conv/global signal to mix in
        self.gate_proj = CastedLinear(dim, dim, bias=False)
        # Value projection: transforms input before mixing
        self.val_proj = CastedLinear(dim, dim, bias=False)
        # Output projection
        self.out_proj = CastedLinear(dim, dim, bias=False)
        self.out_proj._zero_init = True
        # Learnable weight for cumulative mean vs conv balance
        self.global_weight = nn.Parameter(torch.tensor(0.1, dtype=torch.float32))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Causal depthwise conv (left-pad for causality)
        vt = v.transpose(1, 2)  # [B, D, T]
        vt = F.pad(vt, (self.kernel_size - 1, 0))  # causal left-pad
        conv_out = self.conv(vt).transpose(1, 2)  # [B, T, D]
        # Causal cumulative mean for global context
        positions = torch.arange(1, seqlen + 1, device=x.device, dtype=x.dtype).unsqueeze(0).unsqueeze(-1)
        cum_mean = v.cumsum(dim=1) / positions  # [B, T, D]
        # Combine local (conv) and global (cum_mean) with learned balance
        gw = self.global_weight.to(dtype=x.dtype)
        mixed = (1 - gw) * conv_out + gw * cum_mean
...
class Block(nn.Module):
    ...
    use_conv: if True, replaces attention with GatedConvMixer."""
    def __init__(
        self,
        dim: int,
        num_heads: int,
        ...
        if use_conv:
            self.attn = GatedConvMixer(dim, kernel_size=conv_kernel)
        else:
...
class GPT(nn.Module):
    def __init__(
        ...
        # CONV_BLOCKS: "all" for all conv, or comma-separated indices (e.g., "0,1,2")
        conv_str = os.environ.get("CONV_BLOCKS", "")
        conv_kernel = int(os.environ.get("CONV_KERNEL", 15))
        ...
        if num_shared > 0:
            # FiLM-depth: N shared blocks, cycled to effective_depth with per-layer modulation
            self.shared_mode = True
            self.num_shared = num_shared
            self.effective_depth = effective_depth
            if conv_str == "all":
                conv_set = set(range(num_shared))
            elif conv_str.strip():
                conv_set = set(int(x) for x in conv_str.split(","))
            else:
                conv_set = set()
            self.blocks = nn.ModuleList([
                Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                      use_conv=(i in conv_set), conv_kernel=conv_kernel)
                for i in range(num_shared)
            ])
            self.film = FiLMDepthParams(model_dim, effective_depth)
            self.block_schedule = [i % num_shared for i in range(effective_depth)]
        else:
            # Standard: independent blocks
