...
class MLP(nn.Module):
    # Factored MLP: two sequential sub-MLPs with relu^2, same param budget as one wide MLP
    # Standard 8x: 512→4096→512 = 2 * 512 * 4096 = 4.19M params
    # Factored: 512→2048→512 + 512→2048→512 = 4 * 512 * 2048 = 4.19M params (same!)
    # But deeper nonlinearity from two relu^2 stages
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
        use_factored = bool(int(os.environ.get("FACTORED_MLP", "0")))
        self.factored = use_factored
        if use_factored:
            half_hidden = (mlp_mult * dim) // 2
            self.fc1 = CastedLinear(dim, half_hidden, bias=False)
            self.proj1 = CastedLinear(half_hidden, dim, bias=False)
            self.fc2 = CastedLinear(dim, half_hidden, bias=False)
            self.proj2 = CastedLinear(half_hidden, dim, bias=False)
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.factored:
            ...
            x = x + self.proj1(h.square())  # residual through first sub-MLP
            h = torch.relu(self.fc2(x))
            return self.proj2(h.square())
