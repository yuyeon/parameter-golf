...
class CausalSelfAttention(nn.Module):
    """Differential Attention: each head splits into 2 sub-heads, attention maps are
    subtracted to cancel noise. Same total Q/K parameters, ~4 extra floats per head."""
    def __init__(
        self,
        dim: int,
        num_heads: int,
        ...
        self.sub_dim = self.head_dim // 2  # each sub-head is half the head dim
        if self.head_dim % 2 != 0:
            raise ValueError("head_dim must be even for RoPE and DiffAttn")
        # Partial RoPE applies to sub_dim, so adjust
        self.rope_dims = min(rope_dims, self.sub_dim) if rope_dims > 0 else 0
        ...
        self.rotary = Rotary(self.sub_dim, base=rope_base, train_seq_len=train_seq_len,
                             rope_dims=self.rope_dims)
        # Differential lambda: reparameterized as exp(lq1)*exp(lk1) - exp(lq2)*exp(lk2)
        self.lambda_q1 = nn.Parameter(torch.zeros(num_heads, dtype=torch.float32))
        self.lambda_k1 = nn.Parameter(torch.zeros(num_heads, dtype=torch.float32))
        self.lambda_q2 = nn.Parameter(torch.zeros(num_heads, dtype=torch.float32))
        self.lambda_k2 = nn.Parameter(torch.zeros(num_heads, dtype=torch.float32))
        # Output group norm (per the DIFF Transformer paper)
        self.sub_norm = nn.GroupNorm(num_heads, dim, eps=1e-5, affine=False)

    def forward(self, x: Tensor) -> Tensor:
        bsz, seqlen, dim = x.shape
        sd = self.sub_dim
        # Split Q/K into 2 sub-heads per head: (B, T, 2*H, sd)
        q = self.c_q(x).reshape(bsz, seqlen, self.num_heads, 2, sd)
        k = self.c_k(x).reshape(bsz, seqlen, self.num_kv_heads, 2, sd)
        v = self.c_v(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
        # Normalize and apply RoPE to each sub-head
        q = F.rms_norm(q, (sd,))
        k = F.rms_norm(k, (sd,))
        cos, sin = self.rotary(seqlen, x.device, q.dtype)
        q1, q2 = q[:, :, :, 0, :], q[:, :, :, 1, :]  # (B, T, H, sd)
        k1, k2 = k[:, :, :, 0, :], k[:, :, :, 1, :]  # (B, T, Hkv, sd)
        ...
        # Apply q_gain to both sub-heads
        gain = self.q_gain.to(dtype=q1.dtype)[None, None, :, None]
        q1, q2 = q1 * gain, q2 * gain
        # Two FA3 calls with half-dim heads, same V
        y1 = flash_attn_3_func(q1, k1, v, causal=True)
        y2 = flash_attn_3_func(q2, k2, v, causal=True)
        if isinstance(y1, tuple):
            y1, y2 = y1[0], y2[0]
        # Compute lambda and subtract: y = (1 - lambda) * y1 + lambda * (y1 - y2) = y1 - lambda * y2
        lam = (torch.exp(self.lambda_q1) * torch.exp(self.lambda_k1) -
               torch.exp(self.lambda_q2) * torch.exp(self.lambda_k2))
        lam = lam.to(dtype=y1.dtype)[None, None, :, None]  # (1, 1, H, 1)
        y = y1 - lam * y2  # (B, T, H, head_dim)
        y = y.reshape(bsz, seqlen, dim)
        y = self.sub_norm(y.transpose(1, 2)).transpose(1, 2)  # GroupNorm over dim
        return self.proj(y)
