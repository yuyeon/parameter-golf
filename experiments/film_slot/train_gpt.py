...
class Hyperparameters:
    ...
    # SLOT: test-time adaptation (per-sample delta + logit bias optimization)
...
def eval_val_slot(
    ...
    Optimizes per-sample delta + logit bias on frozen hidden states."""
    ...
    ws_list = [ws for ws in range(0, total_tok, stride) if min(ws + seq_s, total_tok) - ws >= 1]
    ...
    for bi in range(0, len(my_ws), args.slot_batch_seqs):
        ...
        with torch.no_grad():
            ...
            for i, ws in enumerate(bws):
                ...
                prev = xb[i, s:wlen].reshape(-1)
                tgt = yb[i, s:wlen].reshape(-1)
    ...
    tpb = token_count.item() / byte_sum.item()
...
def main() -> None:
    ...
    # SLOT: test-time adaptation on the quantized model
    if args.slot_enabled:
        log0(f"slot:starting steps={args.slot_steps} lr={args.slot_lr} stride={args.eval_stride}")
        torch.cuda.synchronize()
        t_slot = time.perf_counter()
        slot_val_loss, slot_val_bpb = eval_val_slot(
            args, base_model, rank, world_size, device,
        ...
        torch.cuda.synchronize()
        log0(f"final_slot val_loss:{slot_val_loss:.4f} val_bpb:{slot_val_bpb:.4f} "
             f"steps:{args.slot_steps} lr:{args.slot_lr} eval_time:{1000.0*(time.perf_counter()-t_slot):.0f}ms")
